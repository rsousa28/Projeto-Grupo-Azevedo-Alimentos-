...
export const HoldingDebtAnalysis: React.FC<HoldingDebtAnalysisProps> = ({ loans, liabilities, storeBenchmarks, onUpdate }) => {
  ...
  const [paymentAccount, setPaymentAccount] = useState<string>('B28');
  ...
  // Abrir Modal de Pagamento de Dívida
  const handleOpenPayModal = (liab: StoreLiability) => {
    if (liab.status === 'Quitado' || liab.totalAmount <= 0) {
      showToast('Esta dívida já foi totalmente quitada!');
      return;
    }
    setPaymentModalLiability(liab);
    if (liab.monthlyPayment > 0 && liab.monthlyPayment < liab.totalAmount) {
      setPaymentMode('installment');
      setCustomPaymentAmount(formatBRLInput(Math.round(liab.monthlyPayment * 100).toString()));
    } else {
      setPaymentMode('full');
      setCustomPaymentAmount(formatBRLInput(Math.round(liab.totalAmount * 100).toString()));
    }
    setPaymentDate(new Date().toISOString().split('T')[0]);
    const defaultPayer = liab.payerResponsibility || (liab.isClosedStore ? 'B28' : liab.unit);
    setPaymentAccount(defaultPayer);
    setPaymentNotes('');
  };

  // Cálculo do valor que será pago agora e do novo saldo devedor
  const currentPayAmount = useMemo(() => {
    if (!paymentModalLiability) return 0;
    if (paymentMode === 'full') {
      return paymentModalLiability.totalAmount;
    }
    if (paymentMode === 'installment') {
      return Math.min(paymentModalLiability.monthlyPayment, paymentModalLiability.totalAmount);
    }
    return parseBRLToNumber(customPaymentAmount);
  }, [paymentModalLiability, paymentMode, customPaymentAmount]);

  const newBalanceRemaining = useMemo(() => {
    if (!paymentModalLiability) return 0;
    return Math.max(0, Math.round((paymentModalLiability.totalAmount - currentPayAmount) * 100) / 100);
  }, [paymentModalLiability, currentPayAmount]);

  // Confirmar Pagamento de Dívida
  const handleConfirmPayLiability = () => {
    if (!paymentModalLiability) return;
    if (currentPayAmount <= 0) {
      showToast('Informe um valor de pagamento maior que zero.');
      return;
    }

    const isFull = paymentMode === 'full' || currentPayAmount >= paymentModalLiability.totalAmount;
    const decrementInstallment = paymentMode === 'installment' || 
      (paymentModalLiability.monthlyPayment > 0 && currentPayAmount >= paymentModalLiability.monthlyPayment);

    HoldingStorage.payLiability(paymentModalLiability.id, currentPayAmount, {
      isFullSettlement: isFull,
      decrementInstallment,
      paymentDate,
      payerAccount: paymentAccount,
      notes: paymentNotes
    });

    onUpdate();
    const formatted = formatCurrency(currentPayAmount);
    if (isFull) {
      showToast(`Passivo com ${paymentModalLiability.creditor} foi 100% quitado!`);
    } else {
      showToast(`Pagamento de ${formatted} registrado com sucesso para ${paymentModalLiability.creditor}!`);
    }
  ...
  return (
    <div className="space-y-6">
                ...
                {filteredLiabilities.map((liab) => {
                  ...
                  return (
                    <tr key={liab.id} className={`${isDarkMode ? 'hover:bg-[#18181D]' : 'hover:bg-slate-50/80'} transition-colors group`}>
                      ...
                      {/* Ações: Pagar, Editar e Excluir */}
                      <td className="py-4 px-4 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          {!isQuitado ? (
                            <button
                              onClick={() => handleOpenPayModal(liab)}
                              title="Registrar pagamento ou quitar dívida"
                              className={`px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer ${
                                isDarkMode
                                  ? 'bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500 hover:text-slate-950 border border-emerald-500/30'
                            ...
                            <span className={`px-2 py-1 rounded-lg text-[10px] font-bold flex items-center gap-1 whitespace-nowrap border ${
      ...
      <AnimatePresence>
        ...
        {/* Modal de Pagamento / Amortização de Passivo */}
        {paymentModalLiability && (
          <div 
            onClick={() => setPaymentModalLiability(null)}
          ...
          >
            <motion.div
              ...
              exit={{ scale: 0.95, opacity: 0, y: 10 }}
              transition={{ duration: 0.2 }}
              className={`w-full max-w-xl ${isDarkMode ? 'bg-[#17171A] border-[#2B2B30] text-white' : 'bg-white border-slate-200 text-slate-900'} border rounded-3xl shadow-2xl relative overflow-hidden my-6`}
            >
              {/* Header */}
              <div className={`flex items-center justify-between border-b ${isDarkMode ? 'border-[#26262B] bg-[#17171A]' : 'border-slate-100 bg-white'} px-6 py-5`}>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-500 dark:text-emerald-400 flex items-center justify-center shrink-0">
                  ...
                  <div>
                    ...
                    <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-slate-500'} font-medium`}>
                      Abater parcela ou quitar pendência financeira
                    </p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setPaymentModalLiability(null)}
                  className={`p-2 rounded-xl ${isDarkMode ? 'text-slate-400 hover:text-white hover:bg-[#232328]' : 'text-slate-400 hover:text-slate-700 hover:bg-slate-100'} transition-all cursor-pointer`}
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="p-6 space-y-5">
                {/* Info da Dívida */}
                <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-[#1C1C20] border-[#2E2E35]' : 'bg-slate-50 border-slate-200/80'} space-y-2.5`}>
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-500 border border-amber-500/20">
                      {paymentModalLiability.unit}
                    </span>
                  </div>

                  <div className={`grid grid-cols-2 gap-3 pt-2 border-t ${isDarkMode ? 'border-[#292930]' : 'border-slate-200'}`}>
                    <div>
                      <span className="text-[10px] font-semibold text-slate-400 block uppercase">Saldo Restante</span>
                      <span className={`text-sm font-black font-mono ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                        {formatCurrency(paymentModalLiability.totalAmount)}
                      </span>
                    </div>
                    <div>
                      <span className="text-[10px] font-semibold text-slate-400 block uppercase">Parcela Cadastrada</span>
                      <span className="text-sm font-bold font-mono text-amber-500">
                        {paymentModalLiability.monthlyPayment > 0 ? `${formatCurrency(paymentModalLiability.monthlyPayment)}/mês` : 'Sem parcela fixa'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    {/* Opção 1: Pagar Parcela (se houver parcela mensal cadastrada menor que o total) */}
                    {paymentModalLiability.monthlyPayment > 0 && paymentModalLiability.monthlyPayment < paymentModalLiability.totalAmount && (
                      <button
                        type="button"
                        onClick={() => {
                          setPaymentMode('installment');
                          setCustomPaymentAmount(formatBRLInput(Math.round(paymentModalLiability.monthlyPayment * 100).toString()));
                        }}
                        className={`p-3 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between ${
                          paymentMode === 'installment'
                            ? 'border-emerald-500 bg-emerald-500/10 shadow-xs'
                            : isDarkMode
                              ? 'border-[#2E2E35] bg-[#1C1C20] hover:border-slate-600'
                              : 'border-slate-200 bg-white hover:border-slate-300'
                        }`}
                      >
                        <div>
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-black uppercase text-emerald-500">1 Parcela</span>
                            {paymentMode === 'installment' && <Check className="w-3.5 h-3.5 text-emerald-500" />}
                          </div>
                          <div className={`text-xs font-bold mt-1 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                            Pagar Parcela
                          </div>
                        </div>
                        <div className="text-xs font-black font-mono text-emerald-500 mt-2">
                          {formatCurrency(paymentModalLiability.monthlyPayment)}
                        </div>
                      </button>
                    )}

                    {/* Opção 2: Quitar Totalidade */}
                    <button
                      type="button"
                      onClick={() => {
                        setPaymentMode('full');
                        setCustomPaymentAmount(formatBRLInput(Math.round(paymentModalLiability.totalAmount * 100).toString()));
                      }}
                      className={`p-3 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between ${
                        paymentMode === 'full'
                          ? 'border-emerald-500 bg-emerald-500/10 shadow-xs'
                    ...
                    >
                      <div>
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-black uppercase text-emerald-500">Quitação</span>
                          {paymentMode === 'full' && <Check className="w-3.5 h-3.5 text-emerald-500" />}
                        </div>
                        <div className={`text-xs font-bold mt-1 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                          Quitar Tudo (100%)
                        </div>
                    ...
                    >
                      <div>
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-black uppercase text-emerald-500">Parcial</span>
                          {paymentMode === 'custom' && <Check className="w-3.5 h-3.5 text-emerald-500" />}
                        </div>
                        <div className={`text-xs font-bold mt-1 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                          Outro Valor
                        </div>
                ...
                {/* Campo de valor caso seja custom */}
                {paymentMode === 'custom' && (
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) => setCustomPaymentAmount(formatBRLInput(e.target.value))}
                        placeholder="R$ 0,00"
                        className={`w-full px-4 py-3 rounded-xl border font-mono font-bold text-base ${
                          isDarkMode 
                            ? 'bg-[#141416] border-[#2F2F36] text-white focus:border-emerald-500' 
                ...
                {/* Resumo visual do abatimento */}
                <div className={`p-4 rounded-2xl border ${
                  newBalanceRemaining === 0 
                    ? isDarkMode ? 'bg-emerald-950/20 border-emerald-500/40' : 'bg-emerald-50 border-emerald-300'
                    : isDarkMode ? 'bg-[#18181C] border-[#2A2A30]' : 'bg-slate-50 border-slate-200'
                } flex items-center justify-between`}>
                  <div>
                    <span className="text-[10px] font-bold text-slate-400 block uppercase">Novo Saldo Devedor</span>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className={`text-base font-black font-mono ${newBalanceRemaining === 0 ? 'text-emerald-500' : isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                        {formatCurrency(newBalanceRemaining)}
                      </span>
                      {newBalanceRemaining === 0 && (
                  ...
                  <div className="text-right">
                    <span className="text-[10px] font-bold text-slate-400 block uppercase">Total Pago Agora</span>
                ...
                {/* Campos adicionais: Data, Conta/Unidade Pagadora, Observação */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2.5 rounded-xl border text-xs font-medium ${
                        isDarkMode ? 'bg-[#141416] border-[#2F2F36] text-white' : 'bg-white border-slate-300 text-slate-900'
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      <option value="B28">B28 (Rio Mar)</option>
                      <option value="B32">B32 (Mossoró)</option>
                      <option value="VERO">Vero</option>
                      <option value="HOLDING">Caixa Central Holding</option>
                      <option value="RATEIO">Rateio entre Lojas</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="Ex: Pago via PIX pelo Santander, lote 192..."
              ...
              {/* Footer / Ações */}
              <div className={`flex items-center justify-end gap-3 px-6 py-4 border-t ${isDarkMode ? 'border-[#26262B] bg-[#17171A]' : 'border-slate-100 bg-white'}`}>
                <button
                  type="button"
                  onClick={() => setPaymentModalLiability(null)}
                  className={`px-4 py-2.5 rounded-xl ${isDarkMode ? 'text-slate-400 hover:text-white hover:bg-[#202024]' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100'} text-xs font-bold transition-colors cursor-pointer`}
                ...
                <button
                  type="button"
                  onClick={handleConfirmPayLiability}
                  disabled={currentPayAmount <= 0}
                  className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-2 transition-all cursor-pointer shadow-lg ${
                    currentPayAmount <= 0
                ...
                >
                  ...
                  <span>Confirmar Pagamento de {formatCurrency(currentPayAmount)}</span>
                </button>
              </div>
            </motion.div>
          </div>
